...
tFig8_1a = 
    let 
        ...
        v3= Var 3
    in
    Hoja ([Oand v1 (Oor v2 v3)],Sep,[Oor (Oand v1 v2) v3])
tFig8_1b :: Tableau
tFig8_1b =
    let 
        v1= Var 1
        ...
        v3= Var 3
    in
    Hoja ([Oor(Oand v1 v2) v3],Sep,[Oand v1 (Oor v2 v3)])
--
...
tFig8_2a =
    let 
        ...
        v2= Var 2
    in
    Hoja ([Oand(Oneg v1) (Oneg v2)],Sep,[Oneg(Oand v1 v2)])
-- tFig8_2b :: Tableau
tFig8_2b :: Tableau
tFig8_2b =
    let 
        ...
        v2= Var 2
    in
    Hoja ([Oneg(Oand v1 v2)],Sep,[Oand(Oneg v1) (Oneg v2)])
--
...
checkTableau t = case t of
  ...
  (DosRamas s@(v,ts,f) ConD ti td) -> ts == Sep && -- El sequent no debe estar abierto o cerrado
                                      ...
                                      elem a vhi && -- Verificamos que la primera parte de la conjuncion esta en la rama izquierda
                                      elem b vhd && -- Verificamos que la segunda parte de la conjuncion esta en la rama derecha
  ...
  (UnaRama s@(v,ts,f) DisD t) -> ts == Sep && -- El seuquent no debe estar abierto o cerrado
                                 length v + 1 == length vh  && -- La lista resultante debe tener un elemento más ([Oor v1 v2], [v1, v2])
                                 disjFaltante cf avh && -- Verificamos que se elimino de manera correcta la disyunción
                                 checkTableau t -- Revisamos la rama
                                 where
                                   cf = elementosFaltantes v vh
  ...
  (DosRamas s@(v,ts,f) ImpI ti td) -> ts == Sep && -- El sequent no debe estar abierto o cerrado
                                      length v == length vhi && -- Verificamos que sean del mismo tamaño
                                      length v == length vhd && -- Verificamos que sean del mismo tamaño
                                      dfi == dfd && -- Verificamos que falten los mismo elementos
                                      elem a vhi && -- Verificamos que la primera parte de la implicacion esta en la rama izquierda
                                      elem b vhd && -- Verificamos que la segunda parte de la implicacion esta en la rama derecha
                                      checkTableau ti && -- Revisamos la rama izquierda
                                      checkTableau td -- Revisamos la rama derecha
                                      where
                                        vhi = obtenListaIzquierda ti
  ...
  (UnaRama s@(v,ts,f) ImpD t) -> ts == Sep && -- El seuquent no debe estar abierto o cerrado
                                 length v + 1 == length vh  && -- La lista resultante debe tener un elemento más ([Oimp v1 v2], [v1, v2])
                                 impFaltante cf avh && -- Verificamos que se elimino de manera correcta la implicacion
                                 checkTableau t -- Revisamos la rama
                                 where
                                   cf = elementosFaltantes v vh
  ...
  (UnaRama s@(v,ts,f) NegI t) -> ts == Sep && -- El seuquent no debe estar abierto o cerrado
                                 length v + 1 == length vh  && -- La lista resultante debe tener un elemento más ([Oimp v1 v2], [v1, v2])
                                 negFaltante cf avh && -- Verificamos que se elimino de manera correcta la negación
                                 checkTableau t -- Revisamos la rama
                                 where
                                   cf = elementosFaltantes v vh
...
conjFaltante l1 l2 = length l1 == 1 &&
                     or [elem x l2 | x <- l1]
-- Función que nos regresa los elementos de una conjunción
elementosCon :: PL -> (PL,PL)
elementosCon phi = case phi of
  Oand alpha beta -> (alpha, beta)
  _ -> error $ "No es una conjuncion"
  
-- Función que nos dice las disjunciones posibles de una lista
disjPosibles :: [PL] -> [PL]
disjPosibles f = [Oor alpha beta | alpha <- f, beta <- f, alpha /= beta]

-- Función que nos dice si la disjuncion fue eliminada correctamente
disjFaltante :: [PL] -> [PL] -> Bool
disjFaltante l1 l2 = length l1 == 1 &&
                     or [elem x l2 | x <- l1]
-- Función que nos regresa los elementos de una disjuncion
elementosDis :: PL -> (PL,PL)
elementosDis phi = case phi of
  Oor alpha beta -> (alpha, beta)
  _ -> error $ "No es una disjuncion"

-- Función que nos dice las implicaciones posibles de una lista
impPosibles :: [PL] -> [PL]
impPosibles f = [Oimp alpha beta | alpha <- f, beta <- f, alpha /= beta]

-- Función que nos dice si la implicacion fue eliminada correctamente
impFaltante :: [PL] -> [PL] -> Bool
impFaltante l1 l2 = length l1 == 1 &&
                     or [elem x l2 | x <- l1]
-- Función que nos regresa los elementos de una implicacion
elementosImp :: PL -> (PL,PL)
elementosImp phi = case phi of
  Oimp alpha beta -> (alpha, beta)
  _ -> error $ "No es una implicacion"

-- Función que nos dice las negaciones posibles de una lista
negPosibles :: [PL] -> [PL]
