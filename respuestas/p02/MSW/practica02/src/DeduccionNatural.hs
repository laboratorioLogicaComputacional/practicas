...
data ReglaDN = 
            ...
            | Iimp  Int Int | Eimp Int Int |Simp Int    -- reglas para la implicacion
...
checkPaso lprem lpp p = -- listaDePremisas listaDePasosPrevios pasoActual
    case p of
        ...
        (m,(g `Oor` _,Idis1 i,lc)) ->   lpp/=[]                  -- hay pasos previos
                                        ...
                                        && lc== lcN         -- las cajas no cambiaron
                                        && usableP i lc nN  -- i es usable, i<nN && i no esta en una caja cerrada 
                                        && g==fi    -- introduccion de la conjuncion: fi |- fi || fj
                                        where 
                                           fi= phiPasoNum i lpp -- paso i
        (m,(_ `Oor` h,Idis2 j,lc)) ->   lpp/=[]                  -- hay pasos previos
                                        ...
                                        && lc== lcN         -- las cajas no cambiaron
                                        && usableP j lc nN  -- j es usable, j<nN && j no esta en una caja cerrada 
                                        && h==fj    -- introduccion de la conjuncion: fj |- fi | fj
                                        where 
                                           fj= phiPasoNum j lpp -- paso i
        (m,(h,Edis i j k,lc)) ->        lpp/=[]                  -- hay pasos previos
                                        && m==nN+1          -- m se incrementa en 1.
                                        ...
                                        && h==fj            -- introduccion de la implicacion: i->k j->k |- k->fj 
                                            where 
                                            fj= phiPasoNum k lpp
        ...
        (m,(g,E2neg i,lc))           -> lpp/=[]              -- hay pasos previos
                                        ...
                                        && lc== lcN             -- las cajas no cambiaron
                                        && usableP i lc nN      -- i es usable, i<nN && i no esta en una caja cerrada 
                                        && fi==fi     -- eliminacion de la doble negacion: ¬¬fi|- fi 
                                            where 
                                            fi= phiPasoNum i lpp -- paso i
