...
-- Deduccion Natural:
--
data ReglaDN = -- %Cada entrada Int sirve para indicar el número de los pasos donde se aplica la regla. 
...
type Paso   = (PL,ReglaDN,[Caja])   -- Un paso de una prueba,
                                    -- (formula,regla_aplicada,listaDeCajas) %La lista de cajas es para llevar el control de cajas abiertas y cerradas.
type NumPaso= (Int,Paso)            -- Un paso numerado, (numero, paso)
-- %Supongo que devuelve la fórmula en el índice i de la lista de pasos.
...
phiPasoNum i lpasos = case mpi of
    ...
    where
    mpi = lookup i lpasos
-- %A partir de una lista de pasos devuelve el último paso de esa lista.
...
eqCon2 g f = case f of
              _ `Oand` f2   -> g == f2
              _             -> False
-- %Dice si podemos usar un paso en el indice j a partir de pasarle el indice del ultimo paso previo.
...
-- %Supongo que cierra (pone el índice de la caja del paso en que se cierra la caja) y verifica si se puede cerrar la caja.
...
-- % Dice si una f en la PL tiene como operador principal un or y devuelve como respuesta lo que se espqecifica abajo.
esDisyuncion :: PL-> (Bool,PL,PL)
--Regresa (True,g,h) si f= g v h.
esDisyuncion f = case f of
                      g `Oor`h -> (True,g,h)
                      _         -> (False,Bot,Bot)
-- %Dice si es válido el paso efectuado p.
...
checkPaso lprem lpp p = -- listaDePremisas listaDePasosPrevios pasoActual
    case p of
        ...
        (m,(g `Oand` h,Icon i j,lc)) -> lpp/=[]          -- hay pasos previos %Vemos que se hayan hecho pasos previos.
                                        && m==nN+1          -- m se incrementa en 1. %Vemos si el paso que se está realizando coincide con el último paso echo más uno.
                                        && lc== lcN         -- las cajas no cambiaron %Vemos si tenemos las mismas cajas.
                                        && usableP i lc nN  -- i es usable, i<nN && i no esta en una caja cerrada %Vemos que i sea usable para poder aplicar introducción de conjunción
                                        && usableP j lc nN  -- j es usable, j<nN && j no esta en una caja cerrada %Lo mismo que en el anterior.
                                        && g==fi && h==fj   -- introduccion de la conjuncion: fi,fj |- fi & fj %Vemos si la nueva fórmula creada con la introd. del and es igual operando  de la nueva fórmula a operando la fŕomula en los pasos anteriores.
                                            where
        ...
        (m,(g,Econ1 i,lc))           -> lpp/=[]          -- hay pasos previos
                                        ...
                                        && usableP i lc nN  -- i es usable, i<nN && i no esta en una caja cerrada %Vemos si podemos usar la fórmula en el paso i para poder sacar el operando que queremos de esa fórmula.
                                        && g `eqCon1` fi    -- g es el conyunto 1 de fi: gi & hi |- gi %Vemos si g es parte de la fórmula i que está em los pasos previos.
                                            where
        ...
        (m,(Bot,Eneg i j,lc))        -> lpp/=[]              -- hay pasos previos
                                        ...
                                        && fj==fi `Oimp` Bot    -- eliminacion de la negacion: fi,fi->Bot |- Bot
                                            ...
                                            fj= phiPasoNum j lpp -- paso j
        --Reglas para doble negacion % YO LO HICE --&& fj==fi `Oneg` `Oneg`  Bot    -- eliminacion de la doble negacion: fi |- ¬¬fi 
        (m,(f,E2neg i,lc))          -> lpp/=[]              -- hay pasos previos
                                        && m==nN+1              -- m se incrementa en 1.
                                        && lc== lcN             -- las cajas no cambiaron
                                        && usableP i lc nN      -- i es usable, i<nN && i no esta en una caja cerrada
                                        && fi== Oneg (Oneg f)  --Oneg (Oneg f)   -- eliminacion de la doble negacion: fi |- ¬¬fi
                                            where
                                            fi= phiPasoNum i lpp -- paso i
